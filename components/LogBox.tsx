...
export default function LogBox() {
    ...
    const handleSubmit = async () => {
        setLoading(true);
        setError("");
        if (!selectedMode || !distance || !start || !end) {
            Alert.alert("Error", "Please fill all fields.");
            setLoading(false);
            return;
        }
        try {
            const res = await setDoc(doc(collection(db, "trips")), {
                selectedMode,
                distance,
                start,
                end,
            ...
            console.log("❌ Trip Logging Error:", error.message);
    ...
    return (
        <SafeAreaView>
            ...
            <View style={{ marginTop: 30 }}>
                <Button title="+ Log Trip" onPress={handleSubmit} />
            </View>
        </View>
    </SafeAreaView>
    );
